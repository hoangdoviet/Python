...
@router.post("/open", response_model=DataResponse[schemas.Discount])
def create_discount(
        *,
        request: Request,
        db: Session = Depends(deps.get_db),
    ...
    discount = crud.discount.create(db, obj_in=discount_in, user=current_user)

    return DataResponse().success_response(request, discount)


@router.put("/{discount_id}", response_model=DataResponse[schemas.Discount])
def update_discount(
        *,
        request: Request,
        db: Session = Depends(deps.get_db),
...
@router.get("/{discount_id}", response_model=DataResponse[schemas.Discount])
def read_discount(
        request: Request,
        discount_id: str,
